...
export function useAttendanceTaking() {
  ...
  const attendanceData = ref<Record<string, AttendanceValue>>({})
  const savedAttendance = ref<Record<string, AttendanceValue>>({})
  const hasSavedForDate = ref<Record<string, boolean>>({})
  ...
  function isRowDirty(studentId: string): boolean {
  ...
  async function fetchStudentsAndAttendance(classId: string): Promise<void> {
    if (!classId) {
      students.value = []
    ...
    try {
      ...
      const initialHasSaved: Record<string, boolean> = {}
  ...
  function handleAttendanceStatusChange(studentId: string, status: string): void {
  ...
  function handleAttendanceNoteChange(studentId: string, note: string): void {
    const currentValue = attendanceData.value[studentId] || getDefaultAttendanceValue()
    attendanceData.value = {
      ...attendanceData.value,
      [studentId]: {
  ...
  async function handleMark(studentId: string): Promise<void> {
  ...
  function handleRevert(studentId: string): void {
    const savedValue = savedAttendance.value[studentId]
  ...
  async function handleCancelSaved(studentId: string): Promise<void> {
  ...
  async function saveStudents(studentList: ClassStudent[], savingMode: 'displayed' | 'all'): Promise<void> {
  ...
  async function handleSaveDisplayed(): Promise<void> {
  ...
  function applyStatusToDisplayed(status: string): void {
