 ...
 */
export function guestOnlyGuard(to: RouteLocationNormalized) {
  const authStore = useAuthStore()

  if (to.meta.guestOnly && authStore.isAuthenticated) {
    return DASHBOARD_ROUTE_BY_ROLE[authStore.currentUserRole as UserRole] || '/'
  }
...
export function roleGuard(to: RouteLocationNormalized) {
  ...
  if (to.meta.roles?.length) {
    ...
    if (!role || !to.meta.roles.includes(role)) {
      return DASHBOARD_ROUTE_BY_ROLE[role as UserRole] || { name: 'login' }
    }
  }
 ...
 */
export async function navigationGuard(to: RouteLocationNormalized) {
  // Run guards in order
